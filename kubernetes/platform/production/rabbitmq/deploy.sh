...
while [ $(kubectl get pod -l app.kubernetes.io/name=grouphq-rabbitmq -n rabbitmq-system | wc -l) -eq 0 ] ; do
  sleep 15 
done
...
kubectl wait \
  --for=condition=ready pod \
  --selector=app.kubernetes.io/name=grouphq-rabbitmq \
  --timeout=600s \
  --namespace=rabbitmq-system
...
export RABBITMQ_USERNAME=$(kubectl get secret grouphq-rabbitmq-default-user -o jsonpath='{.data.username}' -n=rabbitmq-system | base64 --decode)
export RABBITMQ_PASSWORD=$(kubectl get secret grouphq-rabbitmq-default-user -o jsonpath='{.data.password}' -n=rabbitmq-system | base64 --decode)
...
kubectl delete secret grouphq-rabbitmq-credentials || true

kubectl create secret generic grouphq-rabbitmq-credentials \
    --from-literal=spring.rabbitmq.host=grouphq-rabbitmq.rabbitmq-system.svc.cluster.local \
...
echo "\n🍃 Secret 'grouphq-rabbitmq-credentials' has been created for Spring Boot applications to interact with RabbitMQ."
